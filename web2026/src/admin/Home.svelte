<script lang="ts">
    ...
    import { RefreshCw, Plus, Search, Pencil, Eye, EyeOff } from "lucide-svelte"
    ...
    const NewData = (e: string,id: string, name: string, role: string, status: Status,
    ...
    async function HandleSave() {
        const userSchema = z.object({
            ...
            if (data.sData === "Edit" && data.password) {
        ...
        loadingSave = true
        try{
            const res = await apiFetch(path_api+"api/admin/save", token, {
                method: "POST",
                body: JSON.stringify({
                    type: form.sData,
            ...
            if (json.status == 200) {
                ...
                if (json.status === 409 || json.message === "Duplicate Entry") {
                    ...
                    description: `Admin dengan username "${form.field_id}" sudah ada`,
                    })
                    return
                }else{
          ...
          <table class="w-full text-sm min-w-[800px] ">
              <thead class="bg-muted/50 border-b sticky top-0 z-10">
                  <tr>
                      ...
                      <th class="text-left px-4 py-3 font-medium text-muted-foreground w-[5%]">ROLE</th>
                      <th class="text-left px-4 py-3 font-medium text-muted-foreground w-[100%]">NAMA</th>
                      <th class="text-left px-4 py-3 font-medium text-muted-foreground w-[15%]">IPADDRESS</th>
                      <th class="text-center px-4 py-3 font-medium text-muted-foreground w-[15%]">LASTLOGIN</th>
                  </tr>
              </thead>
              <tbody>
                  {#if isLoading}
                  ...
                  {:else}
                      ...
                      <tr class="border-b last:border-0 hover:bg-muted/30 transition-colors">
                          ...
                          <td class="px-4 py-3 ">{rec.home_id}</td>
                          <td class="px-4 py-3">{rec.home_rule}</td>
                          <td class="px-4 py-3">{rec.home_name}</td>
                          <td class="px-4 py-3">{rec.home_ipaddress}</td>
                          <td class="px-4 py-3 whitespace-nowrap">{rec.home_lastlogin}</td>
                      </tr>
                      {/each}
                  {/if}
              </tbody>
  ...
  <Dialog.Content class="max-w-md">
    ...
    <div class="flex flex-col gap-4 py-2">
      <div class="flex flex-col gap-2">
        <Label for="admin-id">Username <span class="text-destructive">*</span></Label>
        <Input
          maxlength="25"
      ...
      <div class="flex flex-col gap-2">
        <Label for="admin-password">Password {#if form.sData=="New"}<span class="text-destructive">*</span>{/if}</Label>
        <div class="relative">
            <Input
                maxlength="20"
      ...
      <div class="flex flex-col gap-2">
        <Label for="admin-role">Role / Jabatan <span class="text-destructive">*</span></Label>
        <Input
          id="admin-role"
          maxlength="30"
          placeholder="Contoh: SUPERADMIN, FINANCE"
          oninput={(e) => handleSanitizeInput(e, "field_role","string_normal")}
          bind:value={form.field_role}/>
      </div>
      <div class="flex flex-col gap-2">
        <Label for="admin-name">Nama <span class="text-destructive">*</span></Label>
        <Input
          id="admin-name"
          maxlength="50"
